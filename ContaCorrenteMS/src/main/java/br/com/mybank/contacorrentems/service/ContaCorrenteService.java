...
import br.com.mybank.contacorrentems.repository.ContaCorrenteRepository;
import br.com.mybank.contacorrentems.repository.TransacoesEmCCRepository;
import br.com.mybank.contacorrentems.response.TransacoesEmCI;
...
public class ContaCorrenteService {
    ...
    public ResponseEntity<?> atualizarContacorrente(ContaCorrente contaCorrente, String token) {
    ...
    public ResponseEntity<?> removerContaCorrente(String token) {
        ...
        try {
            contaCorrente = listarPorIdUsuario((Integer) tokenJson.get("id")).get();
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public ResponseEntity<?> sacar(TransacoesEmCC saque, String token) {
        ...
        Optional<ContaCorrente> contaCorrenteOptional = null;
        try {
            contaCorrenteOptional = listarPorIdUsuario((Integer) tokenJson.get("id"));
        } catch (JSONException e) {
            e.printStackTrace();
        }
        BigDecimal saldoCC = contaCorrenteOptional.get().getSaldoCorrente();
...
//TODO tratar essa resposta de uma maneira melhor, tentar pelo código do retorno fornecido pelo pixAPI
    ...
    public ResponseEntity<?> recarregarCelular(String celular, String token, BigDecimal valor) {
